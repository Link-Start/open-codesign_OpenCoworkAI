...
const {start,staticGate,sha256,browserVersionKey,CALIBRATION_POLICY_SCHEMA,DOMAIN_RULES,OBSERVER_REVISION,channelEncoding,APP_SCRIPT_ENTRY_CONTRACT,HTML_ENTRY_CONTRACT,ENTRY_CONTRACT}=await import('../src/browser-session-r3.mjs');
const {measureUntilDeadline,DEFAULT_MEASUREMENT_POLICY}=await import('../src/measurement-r2.mjs');
const {runLayoutDomainCase}=await import('./layout-domain-controls-r3.mjs');
const {runNumberValueCase}=await import('./number-value-controls-r3.mjs');
...
const output=await mkdtemp(join(root,'out','browser-calibration-r3-'));
const fixtureNames=['value-matrix.html','value-matrix.jsx','value-matrix-legacy.jsx','entry-underscore.jsx','entry-both.jsx','layout-domain-r3.html','layout-domain-r3.jsx','number-value-r3.html','number-value-r3.jsx'];
const inputPaths={observer:join(root,'src','browser-session-r3.mjs'),frozenF0Observer:join(root,'src','browser-session-r2.mjs'),entryHelper:join(root,'src','entry-contract-r2.mjs'),layoutHelper:join(root,'src','layout-facts-r3.mjs'),measurement:join(root,'src','measurement-r2.mjs'),records:join(root,'src','measurement-records-r2.mjs'),runner:fileURLToPath(import.meta.url),domainControls:join(root,'scripts','layout-domain-controls-r3.mjs'),numberControls:join(root,'scripts','number-value-controls-r3.mjs'),browserTests:join(root,'tests','browser-session-r3.test.mjs'),layoutTests:join(root,'tests','layout-facts-r3.test.mjs'),...Object.fromEntries(fixtureNames.map(name=>['fixture:'+name,join(root,'fixtures','handwritten',name)])),...Object.fromEntries(['react.umd.js','react-dom.umd.js','babel.standalone.js'].map(name=>['runtime:'+name,join(runtimeDir,name)]))};
...
const observerSha256=sha256(await readFile(new URL('../src/browser-session-r3.mjs',import.meta.url)));
const layoutHelperSha256=sha256(await readFile(new URL('../src/layout-facts-r3.mjs',import.meta.url)));
...
await writeFile(join(output,'observer-source.mjs'),await readFile(new URL('../src/browser-session-r3.mjs',import.meta.url)),{flag:'wx'});
...
for(const [key,name]of [['layoutHelper','layout-helper-source.mjs'],['domainControls','domain-controls-source.mjs'],['numberControls','number-controls-source.mjs']])await writeFile(join(output,name),await readFile(inputPaths[key]),{flag:'wx'});
const measurementSha256=sha256(await readFile(new URL('../src/measurement-r2.mjs',import.meta.url)));
const report={schema:'browser-calibration-r3-report-1',startedAt,observerRevision:OBSERVER_REVISION,observerSha256,runnerSha256,measurementSha256,entryHelperSha256,layoutHelperSha256,nodeVersion:process.version,output,runKind:'trusted-control-calibration-not-scientific-sampling',inputIntegrity:{paths:inputPaths,startedAt:inputHashesStartedAt,before:inputHashesBefore,after:null,stable:false},policy:DEFAULT_MEASUREMENT_POLICY,cases:[],domainCases:[],numberCases:[],abiCases:[],cost:{wallMs:null,humanMinutes:null,authoringTokens:'unknown',modelCalls:0,networkCalls:0},passed:false};
...
async function runCase(format,viewportName,viewport,calibrationPolicy){
 ...
 async function sample(label,selector,expectedById,{transient=false}={}){
  ...
  for(const e of observation.elements){
   ...
   if(calibrationPolicy&&stable&&e.visible&&e.rawPresence.axNode&&!e.rawPresence.axIgnored&&['input:text','textarea','input:number'].includes(e.type))assert.equal(e.valueState,'available',label+' calibrated visible value');
  }
  return row;
 }
 ...
 try{
  ...
  for(const e of record.qualification.elements)assert.equal(e.valueState,e.type==='input:number'?(e.attributes.id==='n-zero'||calibrationPolicy?'available':'unavailable'):'unavailable');
...
function derivePolicy(){
 ...
 for(const type of ['input:text','textarea']){
  ...
  rules.push({ruleId:'r3-'+type+'-'+browserVersionKey(version),browserVersionKey:browserVersionKey(version),type,emptyMeansEmpty:true,emptyEncoding,validated:true,emptyWitnessCount:empties.length,nonemptyWitnessCount:nonempty.length,scope:'visible AX-backed native text controls; both RareStringData fields exist; native type channel contains stringIndex=-1, other channel entry absent and AX value absent'});
 }
 const numeric=report.numberCases.filter(c=>c.caseId.startsWith('number-discovery-')),domains=report.domainCases.filter(c=>c.caseId.startsWith('domain-discovery-'));
 assert.equal(numeric.length,4);assert.equal(domains.length,4);assert.ok([...numeric,...domains].every(c=>c.passed&&browserVersionKey(c.receipt.browserVersion)===browserVersionKey(version)));
 const numberChecks=numeric.flatMap(c=>c.samples.flatMap(s=>s.checks.map(x=>({...x,caseId:c.caseId})))).filter(x=>x.type==='input:number'&&x.visible&&x.rawPresence.axNode&&!x.rawPresence.axIgnored&&x.stable),emptyNumbers=numberChecks.filter(x=>x.witnessValue===''),nonemptyNumbers=numberChecks.filter(x=>x.witnessValue!=='');
 const numberEmptyEncoding={textValue:'entry-absent',inputValue:'string-index-minus-one',axValue:'absent'},numberSparse=x=>channelEncoding(x.rawPresence.inputValue)==='string-index-minus-one'&&channelEncoding(x.rawPresence.textValue)==='entry-absent'&&!x.rawPresence.axValue.entryPresent;
 assert.ok(emptyNumbers.length>=4&&nonemptyNumbers.length>=4);assert.ok(emptyNumbers.every(numberSparse));assert.ok(nonemptyNumbers.every(x=>!numberSparse(x)&&channelEncoding(x.rawPresence.inputValue)==='string-present'&&x.rawPresence.inputValue.value===x.witnessValue&&x.rawPresence.axValue.type==='number'&&Number.isFinite(x.rawPresence.axValue.value)&&Number(x.witnessValue)===x.rawPresence.axValue.value));
 for(const c of numeric){const values=new Set(nonemptyNumbers.filter(x=>x.caseId===c.caseId).map(x=>x.witnessValue));for(const value of ['0','7','-3','1.5','1e2','00'])assert.ok(values.has(value),c.caseId+' missing numeric lexical control '+value)}
 const numericLexicalWitnessCount=nonemptyNumbers.filter(x=>x.witnessValue!==String(x.rawPresence.axValue.value)).length;assert.ok(numericLexicalWitnessCount>=4);
 rules.push({ruleId:'r3-input:number-'+browserVersionKey(version),browserVersionKey:browserVersionKey(version),type:'input:number',emptyMeansEmpty:true,emptyEncoding:numberEmptyEncoding,validated:true,emptyWitnessCount:emptyNumbers.length,nonemptyWitnessCount:nonemptyNumbers.length,numberAXSemantics:'finite-number-equivalence-preserve-DOM-string',numericLexicalWitnessCount,scope:'visible AX-backed HTMLInputElement number; native DOM string retained; empty sentinel is never zero or absent entry; numeric AX is only a checked semantic witness'});
 const body={schema:CALIBRATION_POLICY_SCHEMA,browserVersion:version,observerSha256,runnerSha256,entryHelperSha256,layoutHelperSha256,domainRules:DOMAIN_RULES,dependencyHashes:inputHashesBefore,discoveryCaseIds:[...discovery,...numeric,...domains].map(c=>c.caseId),fixtureHashes:Object.fromEntries(fixtureNames.map(name=>[name,inputHashesBefore['fixture:'+name]])),runtimeHashes:discovery[0].receipt.runtimeHashes,compilerHashes:discovery.find(c=>c.format==='jsx').receipt.compilerHashes,rules};
 return{...body,policyId:'r3-calibration-'+sha256(JSON.stringify(body))};
}
try{
 assert.equal(inputHashesBefore.frozenF0Observer,'75189d74d8c75c073b79db883b6e2648f72049be2ac4b31adfc06e1ec6ba498c','FROZEN_F0_DRIFT');assert.equal(layoutHelperSha256,inputHashesBefore.layoutHelper);assert.equal(observerSha256,inputHashesBefore.observer);assert.equal(entryHelperSha256,inputHashesBefore.entryHelper);assert.equal(runnerSha256,inputHashesBefore.runner);assert.equal(measurementSha256,inputHashesBefore.measurement);
 ...
 for(const format of ['html','jsx'])for(const[viewportName,viewport]of Object.entries({desktop:{width:1280,height:900},mobile:{width:390,height:844}})){await runNumberValueCase({output,format,viewportName,viewport,records:report.numberCases,inputHashesBefore});await runLayoutDomainCase({output,format,viewportName,viewport,records:report.domainCases,inputHashesBefore})}
 ...
 for(const format of ['html','jsx'])for(const[viewportName,viewport]of Object.entries({desktop:{width:1280,height:900},mobile:{width:390,height:844}})){await runNumberValueCase({output,format,viewportName,viewport,calibrationPolicy:policy,records:report.numberCases,inputHashesBefore});await runLayoutDomainCase({output,format,viewportName,viewport,calibrationPolicy:policy,records:report.domainCases,inputHashesBefore})}
 ...
 console.log(JSON.stringify({output,passed:report.passed,inputHashesStable:report.inputIntegrity.stable,error:report.error?.message,cases:report.cases.map(c=>({caseId:c.caseId,passed:c.passed})),numberCases:report.numberCases.map(c=>({caseId:c.caseId,passed:c.passed})),domainCases:report.domainCases.map(c=>({caseId:c.caseId,passed:c.passed})),abiCases:report.abiCases.map(c=>({name:c.name,passed:c.passed})),wallMs:report.cost.wallMs}));
}
