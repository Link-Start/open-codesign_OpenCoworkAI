// v04 r2 copied from ../../codesign-editability-research-v03/src/browser-session-r1.mjs; historical implementation remains read-only; no legacy imports.
...
export const OBSERVER_REVISION='v04-observer-r2-calibrated-sparse-values-3';
...
export async function start(options={}){
 ...
 let browserVersion=null,rawSequence=0,witnessAllowed=false,renderEpoch=0,renderInFlight=false,cleanupPromise=null,cleanupFailure=null,abortPromise=null; const ownedProcesses=[],operationContext=new AsyncLocalStorage();
  const calibrationPolicy=options.calibrationPolicy?verifyCalibrationPolicy(options.calibrationPolicy):null;
  const observerSha256=sha256(await readFile(new URL(import.meta.url))),entryHelperSha256=sha256(await readFile(new URL('./entry-contract-r2.mjs',import.meta.url)));
 ...
 const command=async(method,params={})=>{const epoch=operationContext.getStore()?.epoch??renderEpoch;if(epoch!==renderEpoch||cleanupPromise||!cdp)fail(closed?'SESSION_CLOSED':'NO_RENDER','Render transport invalidated');const transport=cdp,target=candidateSid??sid;const result=await transport.send(method,params,target);if(epoch!==renderEpoch)fail(closed?'SESSION_CLOSED':'NO_RENDER','Stale render operation');return result};
  const withEpoch=fn=>(...args)=>operationContext.run({epoch:renderEpoch},()=>fn(...args));
 ...
 const ready=()=>{if(closed)fail('SESSION_CLOSED');if(!current)fail('NO_RENDER');if(protocolFailure)fail('CONTAINMENT_CONTROL_FAILED',protocolFailure)};
  ...
  function abortRender(){
   ...
   if(cleanupPromise)return cleanupPromise;
   ++renderEpoch;current=null;protocolFailure='external-observation-abort';candidateSid=null;
 ...
 async function launch(){profile=await mkdtemp(join(options.artifactRoot,'r2-owned-profile-'));child=spawn(options.browserPath,browserArguments(profile),{stdio:'ignore',windowsHide:true});ownedProcesses.push({pid:child.pid,profile,launchedAt:new Date().toISOString(),closedAt:null});let launchError;child.once('error',e=>launchError=e);watchdog=setTimeout(()=>{record('session-watchdog');void abortRender().catch(e=>record('watchdog-cleanup-failed',{message:e.message}))},lifetime);
  ...
  const targets=await cdp.send('Target.getTargets'),target=targets.targetInfos.find(t=>t.type==='page'&&t.url==='about:blank');if(!target)fail('INITIAL_TARGET_MISSING');sid=(await cdp.send('Target.attachToTarget',{targetId:target.targetId,flatten:true})).sessionId;const launchEpoch=renderEpoch;cdp.on(event=>{if(renderEpoch===launchEpoch&&!cleanupPromise)enforce(event)});
  ...
  function valueCollector(request){
   ...
   return async function collect({signal,remainingMs,sequence}={}){
    ...
    try{
     ...
     const identity={renderId:result.renderId,renderEpoch:result.renderEpoch,backendNodeId:e.backendNodeId,contextBackendNodeIds:result.contextBackendNodeIds,type:e.type};
...
export const CALIBRATION_POLICY_SCHEMA='r2-sparse-native-witness-v1';
...
export function decodeValueEvidence({element,snapshotEntry,axNode,browserVersion,calibrationPolicy}){
 ...
 if(!type||!['textarea','input:text','input:search','input:tel','input:url','input:email','input:password'].includes(type))return{...base,valueState:'unsupported',valueError:'VALUE_TYPE_UNSUPPORTED'};
...
export function clonePublicLocator(locator,depth=0){
 if(depth>8||!locator||Object.getPrototypeOf(locator)!==Object.prototype||!['role','text'].includes(locator.by))fail('INVALID_LOCATOR');
 const keys=locator.by==='role'?['by','role','name','exact','within']:['by','text','exact','within'];
 if(Object.keys(locator).some(k=>!keys.includes(k)))fail('INVALID_LOCATOR','Unknown field');
 if(typeof locator[locator.by]!=='string'||locator[locator.by].length>4096||('name'in locator&&(typeof locator.name!=='string'||locator.name.length>4096))||('exact'in locator&&typeof locator.exact!=='boolean'))fail('INVALID_LOCATOR');
 const copy={by:locator.by,[locator.by]:locator[locator.by]};if('name'in locator)copy.name=locator.name;if('exact'in locator)copy.exact=locator.exact;if('within'in locator)copy.within=clonePublicLocator(locator.within,depth+1);return Object.freeze(copy);
}
const verifiedCalibrationPolicies=new WeakSet();
export function validEmptyEncoding(type,encoding){
 return ['input:text','textarea'].includes(type)&&encoding&&Object.keys(encoding).length===3&&encoding.axValue==='absent'&&encoding.textValue===(type==='textarea'?'string-index-minus-one':'entry-absent')&&encoding.inputValue===(type==='input:text'?'string-index-minus-one':'entry-absent');
}
export function verifyCalibrationPolicy(input){
 ...
 if(policy.schema!==CALIBRATION_POLICY_SCHEMA||policyId!=='r2-sparse-'+sha256(JSON.stringify(body)))fail('CALIBRATION_POLICY_DIGEST_MISMATCH');
 const hash=value=>typeof value==='string'&&/^[a-f0-9]{64}$/.test(value);
 if(!hash(policy.observerSha256)||!hash(policy.runnerSha256)||!hash(policy.entryHelperSha256)||!policy.runtimeHashes||!Object.values(policy.runtimeHashes).every(hash)||!policy.fixtureHashes||!Object.values(policy.fixtureHashes).every(hash)||!Array.isArray(policy.rules)||!policy.rules.length)fail('CALIBRATION_POLICY_INVALID');
 ...
 for(const rule of policy.rules){if(types.has(rule.type)||rule.browserVersionKey!==version||rule.validated!==true||rule.emptyMeansEmpty!==true||!validEmptyEncoding(rule.type,rule.emptyEncoding)||!Number.isInteger(rule.emptyWitnessCount)||rule.emptyWitnessCount<4||!Number.isInteger(rule.nonemptyWitnessCount)||rule.nonemptyWitnessCount<4)fail('CALIBRATION_RULE_INVALID');types.add(rule.type)}
