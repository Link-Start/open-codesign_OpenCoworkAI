...
function binomial(n,p){if(p===0)return Array.from({length:n+1},(_,i)=>+(i===0));if(p===1)return Array.from({length:n+1},(_,i)=>+(i===n));const a=Array(n+1).fill(0),mode=Math.floor((n+1)*p);a[mode]=1;for(let k=mode+1;k<=n;k++)a[k]=a[k-1]*(n-k+1)/k*p/(1-p);for(let k=mode-1;k>=0;k--)a[k]=a[k+1]*(k+1)/(n-k)*(1-p)/p;const sum=a.reduce((x,y)=>x+y,0);return a.map(x=>x/sum);}
...
check('Audit numerical stability: 600-trial binomial normalization, mean and reflection',()=>{for(const p of [.005,.05,.5,.95,.995]){const a=binomial(600,p),b=binomial(600,1-p);near(a.reduce((x,y)=>x+y,0),1);near(a.reduce((x,y,i)=>x+i*y,0),600*p,1e-10);for(let i=0;i<=600;i++)near(a[i],b[600-i],1e-12);}});
